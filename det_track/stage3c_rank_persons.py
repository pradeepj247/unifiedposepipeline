...
Part of reorganized pipeline where analysis (3a) → grouping (3b) → ranking (3c).

Usage:
    python stage3c_rank_persons.py --config configs/pipeline_config.yaml
"""
...
def run_ranking(config):
    ...
    # Rank persons
    logger.info(f"Ranking {len(persons)} persons (method: {ranking_config['method']})...")
    t_start = time.time()
    ...
    t_report_save_start = time.time()
    with open(report_path, 'w') as f:
        json.dump(ranking_report, f, indent=2)
    report_save_time = time.time() - t_report_save_start
    ...
    try:
        ...
        sidecar = {
            ...
            'ranking_report_file': str(ranking_report_file),
            'ranking_time': float(t_ranking),
            'npz_save_time': float(npz_save_time),
            'report_save_time': float(report_save_time),
            'num_persons': int(len(persons)),
            'primary_person_id': int(primary_person['person_id']),
...
def main():
    parser = argparse.ArgumentParser(description='Stage 3c: Person Ranking')
    ...
    if not config['pipeline']['stages'].get('stage3c', False):
        logger = PipelineLogger("Stage 3c: Person Ranking", verbose=False)
